...
[dim](Just type commands as shown - any input works for learning!)[/dim]
    ...
    def _simulate_cortex_output(self, command: str, packages: list[str], 
        ...
        if not show_execution:
            ...
            for step in range(1, total_steps + 1):
                ...
                if step == 1:
                    ...
                    self.console.print(f"  Command: [dim]sudo apt install -y {packages[step-2]}[/dim]")
                time.sleep(0.8)
                self.console.print()
            
            self.console.print(f" [cyan]CX[/cyan]  [green]✓[/green] {pkg_str} installed successfully!\n")
            
            # Show installation ID
            self.console.print(f"📝 Installation recorded (ID: {self.installation_id})")
            self.console.print(f"   To rollback: [cyan]cortex rollback {self.installation_id}[/cyan]\n")
    ...
    def _section_smart_install(self) -> bool:
        ...
        self.console.print("Cortex [bold]shows you exactly what it will do[/bold] before making any changes.")
    ...
    def _section_hardware_aware(self) -> bool:
        ...
        self.console.print("Cortex detects your hardware and prevents incompatible installations.\n")
        ...
        # Check for NVIDIA
        has_nvidia = gpu_info and ("NVIDIA" in str(gpu_info.model).upper() or "GTX" in str(gpu_info.model).upper() or "RTX" in str(gpu_info.model).upper())
        
        # Check for AMD (dedicated or integrated Radeon)
        has_amd = gpu_info and ("AMD" in str(gpu_info.model).upper() or "RADEON" in str(gpu_info.model).upper() or "RENOIR" in str(gpu_info.model).upper())
        ...
        if has_nvidia:
            ...
            with self.console.status("[cyan]CX[/cyan] Understanding request...", spinner="dots"):
                time.sleep(0.8)
            with self.console.status("[cyan]CX[/cyan] Checking hardware compatibility...", spinner="dots"):
                time.sleep(1.0)
            
            self.console.print(f" [cyan]CX[/cyan]  [green]✓[/green] NVIDIA GPU detected - CUDA compatible!\n")
            time.sleep(0.5)
            ...
            self.console.print("[cyan]🤖 Cortex suggests:[/cyan] Install ROCm instead (AMD's GPU framework)")
            time.sleep(0.8)
        ...
        self.console.print("Cortex [bold]checks compatibility FIRST[/bold] and prevents problems!\n")
    ...
    def _section_ai_features(self) -> bool:
        ...
        self.console.print("Cortex understands what you [italic]mean[/italic], not just exact syntax.")
...
Install a complete stack with: [cyan]cortex stack webdev[/cyan]
        ...
        self.console.print("Cortex's AI [bold]understands intent[/bold] and provides smart recommendations.\n")
    ...
    def _section_stacks(self) -> bool:
        ...
        self.console.print("One command sets up your [bold]entire development environment[/bold].\n")
    ...
    def _section_history(self) -> bool:
        ...
        self.console.print("Every installation is tracked. You can [bold]review or undo[/bold] any operation.\n")
    ...
    def _section_rollback(self) -> bool:
        ...
        self.console.print(f"Let's undo our webdev stack installation (ID: {self.installation_id}):")
        ...
        self.console.print(f" [cyan]CX[/cyan]  [green]✓[/green] Rollback successful (ID: {rollback_id})\n")
        ...
        self.console.print("Try anything fearlessly - you can always [bold]roll back[/bold] to a clean state.\n")
    ...
    def _show_finale(self) -> None:
        ...
        self.console.print("\n" + "="*70)
        self.console.print("[bold green]🎉 Demo Complete - You've Mastered Cortex Basics![/bold green]")
        ...
        comparison_table = Table(
            title="🆚 Cortex vs Traditional Package Managers",
            show_header=True,
            border_style="cyan"
        )
