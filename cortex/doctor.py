...
class SystemDoctor:
    ...
    def __init__(self) -> None:
        """Initialize the SystemDoctor with empty check lists."""
    ...
    def run_checks(self) -> int:
        ...
        console.print(Panel.fit(
            "[bold cyan]CORTEX SYSTEM CHECK[/bold cyan]",
            border_style="cyan",
            padding=(1, 4)
        ))
        console.print()
    ...
    def _print_check(
        ...
        console.print(
            f" [cyan]CX[/cyan]  [{color}]{symbol} {prefix}[/{color}] {message}"
        )
    ...
    def _check_python(self) -> None:
        ...
        if sys.version_info >= (3, 10):
            ...
            self._print_check("WARN",f"Python {version} (3.10+ recommended)",
                "Upgrade Python: sudo apt install python3.11")
        else:
            self._print_check("FAIL",f"Python {version} (3.10+ required)",
    ...
    def _check_dependencies(self) -> None:
        ...
        if not missing:
            self._print_check("PASS", "Python dependencies installed")
        elif len(missing) < 3:
            self._print_check(
                "WARN",
            ...
            self._print_check(
                "FAIL",
                f"Missing {len(missing)} dependencies: {', '.join(missing)}",
                "Install dependencies: pip install -r requirements.txt"
            )
    ...
    def _check_gpu_driver(self) -> None:
        ...
        self._print_check(
            ...
            "Optional: Install GPU drivers for acceleration"
        )
    ...
    def _check_disk_space(self) -> None:
        ...
        try:
            ...
            if free_gb > 20:
                self._print_check(
                    "PASS",
                    f"{free_gb:.1f}GB free disk space ({total_gb:.1f}GB total)"
                )
            elif free_gb > 10:
                self._print_check(
                    "WARN",
            ...
            self._print_check(
                "WARN",
                f"Could not check disk space: {type(e).__name__}"
            )
    ...
    def _print_summary(self) -> None:
        ...
        if self.passes:
            table.add_row(
                "[green]✓ Passed[/green]",
                f"[green]{len(self.passes)}[/green]"
            )
        if self.warnings:
            table.add_row(
                "[yellow]⚠ Warnings[/yellow]",
                f"[yellow]{len(self.warnings)}[/yellow]"
            )
        if self.failures:
            table.add_row(
                "[red]✗ Failures[/red]",
                f"[red]{len(self.failures)}[/red]"
            )
