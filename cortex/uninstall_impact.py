...
class DependencyGraphBuilder:
        ...
        if success:
            ...
            for line in lines[1:]:
                ...
                if line and not line.startswith("|"):
                    # Remove any leading pipe or spaces
                    dep = line.lstrip("| ")
    ...
    def get_transitive_dependents(
        ...
        """Get all transitive dependents (packages that depend on this, recursively)"""
        ...
        while current_level and depth < max_depth:
            ...
            for pkg in current_level:
                deps = self.get_reverse_dependencies(pkg)
                for dep in deps:
                    if dep not in all_dependents and dep != package_name:
        ...
        return list(all_dependents), depth


class ServiceImpactMapper:
    """
    Maps packages to system services and provides service status information.
    """
    ...
    # Critical services that should trigger warnings
    CRITICAL_SERVICES = {"ssh", "sshd", "systemd", "dbus", "networking", "NetworkManager"}
    ...
    def get_services_for_package(self, package_name: str) -> list[ServiceInfo]:
        ...
        if package_name in self.PACKAGE_SERVICE_MAP:
            for service_name in self.PACKAGE_SERVICE_MAP[package_name]:
                if "*" in service_name:
                    # Handle pattern services like postgresql@*
                    expanded = self._expand_service_pattern(service_name)
                    for svc in expanded:
                        ...
                        services.append(
                            ServiceInfo(
                                ...
                                is_critical=svc in self.CRITICAL_SERVICES,
                            )
                        )
                else:
        ...
        # Try to find services from package files
