...
class PreflightChecker:
    ...
    def check_disk_space(self, additional_paths: list[str] | None = None) -> list[DiskInfo]:
    ...
    def _get_filesystem_type(self, path: str) -> str:
        ...
        success, output = self._run_shell_command(
            f"df -T {shlex.quote(path)} | tail -1 | awk '{{print $2}}'"
        )
        if success and output:
            return output
        return "unknown"

    def check_package(self, name: str, version_cmd: list[str] | None = None) -> PackageInfo:
        ...
        return PackageInfo(name=name, installed=installed, version=version, path=path)
        ...
        try:
            ...
            try:
                ...
                fallback_key = os.environ.get(
                    "ANTHROPIC_API_KEY" if fallback_provider == "claude" else "OPENAI_API_KEY"
                )
                if fallback_key:
                    from LLM.interpreter import CommandInterpreter

                    self._llm_client = CommandInterpreter(
                        api_key=fallback_key, provider=fallback_provider
                    )
                    return self._llm_client
            except Exception:
...
If unsure, estimate typical sizes. ONLY OUTPUT THE JSON OBJECT."""
                ...
                if client.provider.value == "openai":
                    response = client.client.chat.completions.create(
                        ...
                        messages=[
                            {
                                "role": "system",
                                "content": "You are a Linux package expert. Provide accurate package information in JSON format only.",
                            },
            ...
            self.report.warnings.append(
                f"Failed to parse LLM response for package info: {e}. Falling back to estimated package sizes."
            )
            pass
            ...
            if self.api_key:
                ...
                if pkg_info["packages"]:
                    ...
                    self.report.packages_to_install.append(
                        {"name": "docker-ce", "version": "latest", "size_mb": "~85 (estimate)"}
                    )
                ...
                self.report.config_changes.append("/etc/group (add docker group)")
                self.report.warnings.append(
                    "Package sizes are estimates - provide API key for accurate sizes"
                )
        ...
        if not pkg.installed:
            ...
            if self.api_key:
                ...
                if pkg_info["packages"]:
                    ...
                    self.report.packages_to_install.append(
                        {"name": "containerd.io", "version": "latest", "size_mb": "~45 (estimate)"}
                    )
            else:
                self.report.packages_to_install.append(
                    {"name": "containerd.io", "version": "latest", "size_mb": "~45 (estimate)"}
        ...
        if not pkg.installed:
            ...
            if self.api_key:
                ...
                if pkg_info.get("packages"):
                    ...
                    self.report.packages_to_install.append(
                        {"name": software_name, "version": "latest", "size_mb": "~50 (estimate)"}
                    )
                    self.report.warnings.append(
                        f"Could not fetch real package size for {software_name}"
                    )
            else:
    ...
    def calculate_requirements(self, software: str) -> None:
        ...
        root_disk = next(
            (
                d
                for d in self.report.disk_usage
                if os.path.normcase(os.path.abspath(d.path))
...
def format_report(report: PreflightReport, software: str) -> str:
    ...
    using_estimates = any(
        "estimate" in str(pkg.get("size_mb", "")) for pkg in report.packages_to_install
    )

    # System info
    lines.append("System Information:")
    lines.append(
        f"  OS: {report.os_info.get('distro', 'Unknown')} {report.os_info.get('distro_version', '')}"
    )
    ...
    if report.packages_to_install:
        ...
        for pkg in report.packages_to_install:
            lines.append(
                f"  - {pkg['name']} {pkg.get('version', '')} ({pkg.get('size_mb', '?')} MB)"
            )
